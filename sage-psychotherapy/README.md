...
The three photographs the design is built around are **not in the repository
yet** — the room, the portrait and the logo are stand-in artwork so the site
could be built and reviewed at the right proportions. They render with a visible
"to confirm" treatment and the build warns about them:
...
**Right now the room and portrait are stand-in artwork** generated by
`scripts/make-placeholder-images.mjs`. To go live:

1. Add the real files to `public/images/`, keeping the names and rough shapes:
   - `room.jpg` — landscape, ~3:2. The hero and the pinned scroll.
