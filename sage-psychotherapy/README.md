...
The room photograph is now in place. The **portrait and the logo are still
stand-in artwork** so the site could be built and reviewed at the right
proportions. They render with a visible "to confirm" treatment and the build
warns about them:
...
The room (`public/images/room.jpg`) is Lyndsay's real front room. **The
portrait is still stand-in artwork** generated by
`scripts/make-placeholder-images.mjs`. To finish going live:

1. Add the real portrait to `public/images/`, keeping the name and rough shape:
   - `portrait.jpg` — portrait, ~4:5. Natural light, blue scarf.
   A higher-resolution room photo is also worth swapping in later — the current
   one is a phone shot and the full-bleed hero would sharpen with more pixels.
