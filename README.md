...
- **The Experience section is an editorial run, not three cards.**
  ...
  decoration carrying no information. It's now alternating full-width
  rows (photo left/right, copy opposite), each led by a real photograph
  of the actual room, with the number reduced to a small Anton label.
  Rows reveal from the side their photo sits on, so the alternation
  reads in the motion too.
  **VIP Nights has since been removed** — Eddie's confirmed they don't
  run them any more — so the run is Showcase Events and Private Hire,
  the heading is "Two ways to spend the night", and the booking FAQ no
  longer routes people through a VIP enquiry. `exp-vip.jpg` was deleted
...
- **The header is one lockup for the whole group, and each site has its own
  ...
  `main.js` is per-toggle rather than by a single id, because both venues
  have one and the single-file preview puts both in one document.
- **Tap targets.** An audit across eight viewports (320px to 1920px) found
  controls well under the 44px guideline, including a scroll cue whose link
  was literally 1px wide, 18px social icons, and 15–19px footer and nav
  links. Each is now grown to ~44px with padding plus a matching negative
  margin, or with a transparent pseudo-element where the design hangs off
  the box edge (the text buttons' rule, the nav underline, the cue's
  hairline) and padding would have moved it. Verified by hit-testing points
  inside the intended area rather than by measuring boxes — the boxes are
  unchanged, which is the point. No horizontal overflow at any of the eight
  sizes on either page.
...
- **Photos are all real venue photography now.** The generated
  ...
  that added them): `hero-room*.jpg` and `exp-showcase/private.jpg`.
